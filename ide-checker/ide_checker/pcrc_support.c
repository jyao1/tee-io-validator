/**
 *  Copyright Notice:
 *  Copyright 2024 DMTF. All rights reserved.
 *  License: BSD 3-Clause License. For full text see link: https://github.com/DMTF/spdm-dump/blob/main/LICENSE.md
 **/

#include "ide_checker.h"

#define PCIE_POLY 0x04C11DB7
#define CXL_POLY  0x1EDC6F41

uint32_t reflect32 (uint32_t x)
{
    uint32_t bits;
    uint32_t mask;
    size_t index;

    bits = 0;
    mask = x;
    for (index = 0; index < 32; index++) {
        bits <<= 1;
        bits |= (mask & 1);
        mask >>= 1;
    }

    return bits;
}

uint8_t reverse [256]= {
    ,
};

uint32_t calc_crc32 (const void *data, size_t len, uint32_t poly)
{
    const unsigned char *buffer;
    uint32_t crc;

    crc = 0xFFFFFFFF;
    buffer = (const unsigned char*) data;
    while (len--) {
        crc = crc ^ (reverse[*buffer++] << 24);
        for (int bit = 0; bit < 8; bit++) {
            if (crc & 0x80000000) {
                crc = (crc << 1) ^ poly;
            } else {
                crc = (crc << 1);
            }
        }
    }
    return reflect32 (~crc);
}

/*
  PCIE PCRC

  The polynomial used has coefficients expressed as 04C1 1DB7h
*/
uint32_t pcie_calc_pcrc (uint8_t *data, size_t data_size)
{
    return calc_crc32 (data, data_size, PCIE_POLY);
}

/*
  CXL PCRC

  A polynomial with the coefficients 1EDC 6F41h shall be used for PCRC calculation.
*/
uint32_t cxl_calc_pcrc (uint8_t *data, size_t data_size)
{
    return calc_crc32 (data, data_size, CXL_POLY);
}
